...
std::string
SetAirConditionerIfTemperatureTool::name() const {
    return "set_air_conditioner_if_temperature";
}

std::string
SetAirConditionerIfTemperatureTool::description() const {
    return
        "原子执行温度条件空调控制。"
        "C++读取当前车内温度并确定性比较，"
        "条件成立时设置空调状态并进行写后验证。"
...
nlohmann::json
SetAirConditionerIfTemperatureTool::parametersSchema() const {
    return {
        {"type", "object"},
        {
            "properties",
...
bool
SetAirConditionerIfTemperatureTool::requiresConfirmation() const {
    return true;
}

AgentToolResult
SetAirConditionerIfTemperatureTool::execute(
    ...
    if (
        ...
        return AgentToolResult::failure(
            "operator, threshold_c and enabled are required"
        );
    }
    ...
    const std::string comparison_operator =
        arguments.at("operator").get<std::string>();

    if (!isSupportedTemperatureOperator(comparison_operator)) {
        return AgentToolResult::failure(
            "operator must be one of: gt, ge, lt, le"
        );
    }
    ...
    const double threshold_c =
        arguments.at("threshold_c").get<double>();
    ...
    if (!arguments.at("enabled").is_boolean()) {
        return AgentToolResult::failure(
    ...
    if (!std::isfinite(temperature_c)) {
        return AgentToolResult::failure(
            "environment sensor returned an invalid temperature"
        );
    }
    ...
    const bool state_before =
        device_.airConditionerEnabled();

    bool state_after = state_before;
    bool control_executed = false;

    if (matched) {
        if (!device_.setAirConditionerEnabled(target_enabled)) {
            return AgentToolResult::failure(
        ...
        state_after = device_.airConditionerEnabled();

        if (state_after != target_enabled) {
            return AgentToolResult::failure(
                "temperature condition matched, "
                "but state verification failed"
            );
        }
    }
